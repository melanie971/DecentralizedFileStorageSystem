...
//import FileIcon, {defaultStyles} from 'react-file-icon'; // see here: https://github.com/corygibbons/react-file-icon
...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      //this is supposed to handle change of Metamask account but not sure we need it - does not work for me
  ...
  onDrop = async (file) => {
    try {
      ...
      const result = await ipfs.add(stream); //debugg to see the object structure in the console
      //debugger;
      const timestamp = Math.round(+new Date() / 1000);
      const type = file.name.substr(file.name.lastIndexOf(".")+1); //it will five us the type of the file 
